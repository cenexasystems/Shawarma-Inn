...
export function PageLayout({ title, subtitle, toolbar, action, statistics, children, footer }: PageLayoutProps) {
  return (
    <div className="min-h-screen bg-erp-bg text-erp-text flex flex-col">
      ...
      {/* Header Area */}
      <div className="pt-4 md:pt-6 xl:pt-[32px] px-4 md:px-6 xl:px-[32px] pb-4 md:pb-[24px] max-w-[1440px] w-full mx-auto">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
          <h1 className="text-[28px] md:text-[36px] xl:text-[42px] font-[700] tracking-[-0.03em] leading-[1.05] text-erp-text">
            {title}
          </h1>
          {subtitle && (
      ...
      {/* Main Content Area */}
      <div className="flex-1 px-4 md:px-6 xl:px-[32px] pb-8 md:pb-[48px] flex flex-col gap-4 md:gap-6 xl:gap-[32px] max-w-[1440px] w-full mx-auto">
        {/* Statistics row */}
        {statistics && (

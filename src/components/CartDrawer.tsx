...
export default function CartDrawer({ isOpen, onClose, cartData }: CartDrawerProps) {
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className="p-4 sm:p-8 border-b border-[var(--border)] flex justify-between items-center">
          <div>
        ...
        {/* Items */}
        <div className="flex-1 min-h-0 overflow-y-auto p-4 sm:p-8 space-y-6 sm:space-y-8">
          {cart.length === 0 ? (
            ...
            cart.map((ci: any) => (
              <div key={ci.id} className="flex items-center gap-2 sm:gap-4 group min-w-0">
                ...
                <div className="flex items-center bg-white/5 rounded-full p-1 gap-0.5 sm:gap-2 flex-shrink-0">
                  <button
                    id={`cart-decrease-${ci.id}`}
                    onClick={() => updateQty(ci.id, ci.qty - 1)}
                    className="w-11 h-11 rounded-full flex items-center justify-center hover:bg-white/10 text-lg text-white font-bold"
                  >
                    −
                  </button>
        ...
        {cart.length > 0 && (
          <div
            className="p-4 sm:p-8 bg-[var(--charcoal)] border-t border-[var(--border)] space-y-4 flex-shrink-0"
